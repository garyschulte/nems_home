...
# 20191202 0.3: adapted to armbian-monitor
######################################################################
# version
version=0.2
...
# Get temperature
f = open("/etc/armbianmonitor/datasources/soctemp", "r")
temp = f.read()
print temp
try:
    ...
    #raise
    print("TEMP UNKNOWN: Unable to read '/etc/armbianmonitor/datasources/soctemp'. Try different user or use sudo?")
    sys.exit(3)
finally:
    f.close()
if (unit == 'f'):
